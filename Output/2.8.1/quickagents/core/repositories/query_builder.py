...
class QueryBuilder(Generic[T]):
            ...
            if suffix and key.endswith(suffix):
                ...
                if op == FilterOp.IN:
                    if not isinstance(value, (list, tuple, set)):
                        raise ValueError(f"__in 操作符需要列表/元组/集合，收到: {type(value)}")
                    if len(value) == 0:
    ...
    def _condition_to_sql(self, cond: FilterCondition) -> Tuple[str, Optional[List[Any]]]:
        """将单个条件转为 SQL"""
