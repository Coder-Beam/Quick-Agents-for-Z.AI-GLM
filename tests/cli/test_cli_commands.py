...
class TestMemoryCommands(unittest.TestCase):
    """记忆管理命令测试（通过 UnifiedDB SQLite）"""

    def setUp(self):
        """测试前准备"""
        self.test_dir = tempfile.mkdtemp()
        self.db_path = os.path.join(self.test_dir, "test_unified.db")
    ...
    def test_memory_set_and_get(self):
        ...
        with patch("sys.stdout", new_callable=StringIO):
            with patch("quickagents.cli.main.UnifiedDB", return_value=self.mock_db):
                cmd_memory(args_set)

        # 获取记忆
        self.mock_db.get_memory.return_value = "test_value"
        args_get = MagicMock()
        args_get.action = "get"
    ...
    def test_memory_search(self):
        ...
        mock_result.key = "project.name"
        mock_result.value = "QuickAgents"
        mock_result.memory_type = "factual"
        self.mock_db.search_memory.return_value = [mock_result]
...
class TestModelsCommand(unittest.TestCase):
        ...
        self.test_config = {
            ...
            "providers": {
                "zhipu": {
                    ...
                    "models": {
                        "GLM-5.1": {"recommended": True, "description": "最新GLM模型"},
                        "GLM-5": {"description": "GLM-5模型"},
                    },
                }
            },
            "categories": {"planning": "GLM-5", "execution": "GLM-4.7"},
            "versionUpgrade": {
                "checkUrl": "https://docs.bigmodel.cn/llms.txt",
                "upgradePath": {"GLM-4.7": "GLM-5", "GLM-5": "GLM-5.1"},
            },
        }
    ...
    def test_models_show(self):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch("quickagents.utils.encoding.read_file_utf8", return_value=json.dumps(self.test_config)):
                with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                    cmd_models(args)
                    output = mock_stdout.getvalue()
...
class TestUninstallCommand(unittest.TestCase):
    ...
    def test_uninstall_dry_run_no_data(self):
        ...
        args.keep_opencode = False
        args.force = False

        # 在无任何QA文件的环境中（包括无.opencode、无配置文件）
    ...
    def test_uninstall_keep_config(self):
        """uninstall --keep-opencode 跳过 .opencode/"""
        args = MagicMock()
        args.dry_run = True
        args.keep_data = False
        args.keep_opencode = True
        args.force = False
        ...
        self.assertIn("[SKIP]", output)
        self.assertIn("--keep-opencode", output)
