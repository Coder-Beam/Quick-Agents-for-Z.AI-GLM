...
class ReviewSession:
    ...
    def get_pending(self) -> List[ReviewItem]:
        return [
            item for item in self._items.values() if item.status == ReviewStatus.PENDING
        ]
    ...
    def is_complete(self) -> bool:
        return all(item.status != ReviewStatus.PENDING for item in self._items.values())

    def get_results(self) -> Dict:
        accepted = [
            i for i in self._items.values() if i.status == ReviewStatus.ACCEPTED
        ]
        modified = [
            i for i in self._items.values() if i.status == ReviewStatus.MODIFIED
        ]
        rejected = [
            i for i in self._items.values() if i.status == ReviewStatus.REJECTED
        ]
        return {
