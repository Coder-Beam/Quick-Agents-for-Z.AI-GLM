    ...
    def add(self, task: Task) -> Task:
        ...
        with self.conn_mgr.get_connection() as conn:
            conn.execute(
                ...
                """,
                (
                    data['id'], data['name'], data['priority'], data['status'],
                    data['assignee'], data['start_time'], data['end_time'],
    ...
    def update(self, task: Task) -> Task:
        ...
        with self.conn_mgr.get_connection() as conn:
            conn.execute(
                ...
                """,
                (
                    data['name'], data['priority'], data['status'],
                    data['assignee'], data['start_time'], data['end_time'],
                    data['updated_at'], data['metadata'], data['id']
                )
            )
            conn.commit()
