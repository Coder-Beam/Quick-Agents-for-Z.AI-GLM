...
class KnowledgeExtractor:
    ...
    def extract_from_text(
        self, text: str, source_type: str = "discussion", **kwargs
    ) -> List[KnowledgeNode]:
        """
    ...
    def _extract_from_line(
        self, line: str, source_type: str, threshold: float, kwargs: Dict[str, Any]
    ) -> List[KnowledgeNode]:
        ...
        fact_node = self._try_extract_type(
            line, NodeType.FACT, self.FACT_PATTERNS, source_type, threshold, kwargs
        )
        if fact_node:
    ...
    def import_from_file(self, file_path: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "nodes_created": 0, "error": str(e)}

        source_type = "file"
        source_uri = f"file:///{file_path.replace(os.sep, '/')}"

        nodes = self.extract_from_text(
            content, source_type=source_type, source_uri=source_uri, **kwargs
        )
