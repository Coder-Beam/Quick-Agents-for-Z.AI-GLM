...
from ..types import EdgeType, KnowledgeNode, KnowledgeEdge
...
class RelationDiscovery:
    ...
    def discover_structural_relations(self, node_id: str) -> List[KnowledgeEdge]:
        ...
        for other in all_nodes:
            ...
            if node.feature_id and other.feature_id and node.feature_id == other.feature_id:
                is_related = True
                weight = 0.9
                reason = "same_feature"
            elif node.project_name and other.project_name and node.project_name == other.project_name:
                is_related = True
                weight = 0.7
                reason = "same_project"
            elif node.created_at and other.created_at:
                if (not node.project_name and not other.project_name and
                    not node.feature_id and not other.feature_id):
    ...
    def trace_requirement(
        ...
        node = self._storage.get_node(node_id)
        if not node:
            return {"node_id": node_id, "chains": [], "related": [], "error": "Node not found"}
