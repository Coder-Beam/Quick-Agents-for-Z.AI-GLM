...
#### ConnectionManager Unification Verified

All production modules confirmed using `ConnectionManager` exclusively:
- `ExperienceCompiler` — Experience entries + compiled articles + FTS5
- `YuGongDB` — 7 tables CRUD with `:memory:` fallback
- `SQLiteGraphStorage` — Knowledge nodes/edges + FTS5 + tag indexing
- Zero raw `sqlite3.connect` in production code (only `:memory:` path preserved)

#### Test Fixes (6 → 0 failures)

- **CLI memory tests**: Patch `UnifiedDB` instead of removed `MemoryManager`
- **KG memory_sync test**: UTF-8 encoding on Windows + match actual output format
- **Migration tests**: Use version 006+ to avoid conflict with built-in migration_004

#### Cross-Module Integration (All ✅)

| From | To | Trigger |
|------|----|---------|
| YuGong Loop | Evolution | `on_task_complete()` |
| AuditGuard | Evolution | Feedback auto-flow |
| Evolution | ExperienceCompiler | Query compiled articles |
| Git Hooks | Evolution + AuditGuard | Post-commit dual trigger |
| Git Hooks | Windows Compatible | `sys.executable` + `.bat` wrapper |
...
### Full Architecture Grade-A Fix + Self-Evolution Closed Loop

#### Unified Architecture (Zero Self-Built Connections)

- KG `discover()` attribute name fix + `create_edge()` signature correction
- KG new `knowledge_node_tags` table + SQL JOIN efficient query
- KG discovery all changed to SQL WHERE/FTS5 (zero Python full-scan)
- `DocPipeline.save()` dispatch bug fix (`hasattr` → `isinstance`)
- Evolution Markdown sync `total_usage` → `usage_count` fix
- `restore_feedback_from_md()` auto-discover `.quickagents/feedback/` directory

#### Auto-Compile Closed Loop

- `on_task_complete()` auto-checks `should_compile()` and sets flag
- `run_periodic_optimization()` auto-triggers compile flow
- New `_auto_compile_experiences()` rule-based instant compilation

#### Real Self-Evolution (5-Stage Closed Loop: Collect → Analyze → Compile → Act → Verify)

- **Act**: `modify_skill()` auto-modify Skill files
- **Act**: `inject_context()` inject evolution insights into project context
- **Act**: `adjust_parameters()` auto-adjust evolution parameters
- **Verify**: `verify_evolution()` success rate comparison + rollback suggestion
- **Semantic Analysis**: Evolution queries ExperienceCompiler compiled articles

#### Cross-Module Integration (Interoperability Matrix All ✅)

- YuGong Loop complete → auto-trigger `Evolution.on_task_complete()`
- AuditGuard extract lessons → auto-flow into Evolution feedback
- Evolution `_suggest_fix()` → queries both feedback + compiled_articles
- Git hooks Windows compatible (`sys.executable` + `.bat` wrapper + log file)
- Git hooks dual-trigger Evolution + AuditGuard

## [2.25.3] - 2026-04-07

### Architecture Integrity Fix + ExperienceCompiler SQLite Rewrite

#### P0 Urgent Fixes (Runtime Crashes)

- Fix `qka audit status` crash (KeyError: `today_changes`)
- Fix `qka audit run` crash (AttributeError: `report.passed` → `report.all_passed`)
- Fix `qka audit log` crash (`get_changes_by_session()` not found)
- Add decisions table migration_005 + `add_decision()` method
- YuGong Loop `YuGongLoop` connect to `YuGongDB` for persistence

#### P1 Architecture Fixes (Data Consistency)

- CLI `qka memory` now uses `UnifiedDB` (SQLite) instead of `MemoryManager` (file-only)
- `SkillEvolution.on_task_complete()` auto-calls `ExperienceCompiler.accumulate()`
- `DocumentPipeline.save()` now writes to `KnowledgeSaver` → SQLite
- KG `discover()` results now auto-persist to SQLite
- KG `sync_to_memory()` uses append mode, preserving main memory system
- New `qka knowledge` CLI commands (status/search/discover)

#### P2 Improvements (Restore Path Fixes)

- `restore_decisions_from_md()` now actually parses and inserts into SQLite
- `restore_feedback_from_md()` now actually parses and inserts into SQLite
- `restore_progress_from_json()` removed reference to non-existent method
- New `qka tasks` and `qka progress` CLI commands
